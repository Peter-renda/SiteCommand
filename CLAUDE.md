...
## Training – Modules / Lessons (Nine-Track PM Curriculum + Quizzes)

### Overview
**Training → Training Modules** (the left-nav label is **Training Modules**; the route is still `/training/lessons`, and `/training` now lands here by default instead of on the Practice launcher) is a hand-authored curriculum that teaches new project managers **how to run SiteCommand's workflows** (RFIs, Submittals, Buyout, Change Events, Commitments, Cost & Billing, Field Ops, Closeout, plus Budget, Scheduling, Permits, Quality, Safety, Risk), **the construction concepts those workflows assume** (reading drawings/specs, RCP, MEP, CSI, lifecycle, contract types, retainage), an **Integrated Project Management** management-layer track built from *Construction Project Management: An Integrated Approach* (Fewings & Henjewele, 3rd ed.) — the integrated approach, defining success (time–cost–quality + six variables), ethical leadership, the client business case & Gateway, stakeholder mapping, procurement-route strategy, leadership styles across the life cycle, integrated risk & value management, quality/customer care, digital construction/BIM, sustainable delivery, and structured close-out — and six deeper tracks built from the PM learning curriculum: **Building the Work** (means & methods in build sequence), **Site & Civil** (site analysis, grading, E&S/SWPPP, stormwater/LID, utilities, streets/parking, ADA routes, landscape, wetlands, ESAs, entitlements, design-side PM), **MEP Systems** (every system as schedule logic — the universal activity chain, first/second fix, fire suppression, plumbing, HVAC airside/hydronics, BMS, electrical distribution, low voltage, fire alarm & security, MEP scheduling, startup/Cx), **Contracts & Commercial** (delivery methods, AIA documents, key clauses, subcontract administration, liens/bonds, claims), **Professional Skills** (financial literacy, estimating, leadership, communication, codes, ethics), and **Preconstruction & Field Ops** (reading geotechnical reports, material takeoffs, constructability & value engineering, the Division 00/01 rulebook, running precon/OAC/coordination meetings, BIM clash coordination, site logistics & mobilization, field layout & survey, and a master construction glossary). It is distinct from Best Practice Templates (company-specific policy that feeds the AI) and from Practice (the hands-on sandbox) — Lessons is the read-and-learn curriculum that prepares a user for both.

### Content Model
- Lesson content is **static and curated**, not user-editable or database-stored. Types + helpers + the original 16 core lessons live in `lib/training-lessons.ts`; the newer content is split by track into `lib/training-lessons-process.ts` (workflow/concept additions), `-management.ts`, `-technical.ts`, `-sitework.ts`, `-mep.ts`, `-commercial.ts`, `-foundations.ts`, `-fieldops.ts` — each importing only *types* from the main file (no runtime cycle) and aggregated there into `LESSONS`. Each `Lesson` has an `id`, a `track` (`"workflow" | "concept" | "management" | "technical" | "sitework" | "mep" | "commercial" | "foundations" | "fieldops"`), a `category` (groups lessons within a track), `title`, `summary`, estimated `minutes`, optional `keyTerms`, a `body` of ordered content blocks (`heading` + `paragraphs`/`bullets`/`ordered`), optional `relatedLessonIds` (cross-links, rendered as chips), and optional `links` (e.g. a "Practice this in your training sandbox" link to `/training/practice`).
- 92 lessons ship today: 14 **Workflows**, 10 **Concepts**, 13 **Integrated PM** (`lib/training-lessons-management.ts` — the integrated approach/why construction fragments, defining success, ethical leadership, the client business case & Gateway, stakeholder mapping, choosing a procurement route, leadership styles across the life cycle, integrated risk & value management, quality & customer care, digital construction/BIM, sustainable delivery, project close-out & systems improvement, plus a **Capstone** case-study module (`pm-cases`) with a scenario-based quiz; six lessons carry "Case in point" blocks drawn from real projects — King Shaka Airport, the Considerate Constructor Scheme, London 2012, Hong Kong NEC3, Heathrow T5, high-performing sustainable buildings; authored from Fewings & Henjewele's *Construction Project Management: An Integrated Approach*, 3rd ed.), 11 **Building the Work** (sitework → foundations/concrete → steel/framing → envelope → MEP/fire → finishes/elevators-low-voltage → testing & commissioning), 12 **Site & Civil**, 11 **MEP Systems** (the pattern → wet systems → HVAC → power & signal → running the MEP job), 6 **Contracts & Commercial**, 6 **Professional Skills**, 9 **Preconstruction & Field Ops** (geotech, takeoffs, constructability/VE, the Div 00/01 rulebook, meetings, BIM clash coordination, site logistics/mobilization, field layout, and a master glossary — built from the Construction Management & Field Execution manual to fill gaps the other tracks don't cover head-on). Id prefixes by track: `wf-`, `cn-`, `pm-`, `tech-`, `sc-`, `mep-`, `com-`, `pf-`, `fo-`.
...
- `app/training/lessons/page.tsx` (session gate + legacy `?lesson=` redirect) + `LessonsClient.tsx` — the module **library**: a 9-way track-tab row (Workflows / Concepts / Integrated PM / Building the Work / Site & Civil / MEP Systems / Contracts & Commercial / Professional Skills / Preconstruction & Field Ops), a category-grouped list where each module is a new-tab hyperlink with a completion checkmark + quiz-grade badge, and header progress counters (modules complete, quizzes taken, average quiz grade). Because modules are completed in **other tabs**, the list silently refetches progress + grades on window `focus`/`visibilitychange` so badges are never stale.
