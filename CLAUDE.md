...
- Every module ends with a short multiple-choice quiz (**3 questions each, 273 total**). Questions + correct answers are static/curated in `lib/training-lesson-quizzes.ts`, keyed by `Lesson.id`, mirroring the lessons themselves. Helpers: `getLessonQuiz`, `getPublicQuiz` (answer-stripped, safe for the client), `hasQuiz`, `gradeQuiz`.
