...
- Every module ends with a short multiple-choice quiz (**3 questions each, 237 total**). Questions + correct answers are static/curated in `lib/training-lesson-quizzes.ts`, keyed by `Lesson.id`, mirroring the lessons themselves. Helpers: `getLessonQuiz`, `getPublicQuiz` (answer-stripped, safe for the client), `hasQuiz`, `gradeQuiz`.
...
- `app/training/lessons/page.tsx` (session gate + legacy `?lesson=` redirect) + `LessonsClient.tsx` — the module **library**: an 8-way track-tab row (Workflows / Concepts / Building the Work / Site & Civil / MEP Systems / Contracts & Commercial / Professional Skills / Preconstruction & Field Ops), a category-grouped list where each module is a new-tab hyperlink with a completion checkmark + quiz-grade badge, and header progress counters (modules complete, quizzes taken, average quiz grade). Because modules are completed in **other tabs**, the list silently refetches progress + grades on window `focus`/`visibilitychange` so badges are never stale.
