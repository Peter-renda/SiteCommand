...
## Training – Modules / Lessons (Lifecycle-Track PM Curriculum + Quizzes)

### Overview
**Training → Training Modules** (the left-nav label is **Training Modules**; the route is still `/training/lessons`, and `/training` now lands here by default instead of on the Practice launcher) is a hand-authored curriculum organized around the **real construction project lifecycle**. The nine tracks are the seven build-order phases — **1. Pre-Construction & Entitlements**, **2. Site Development & Civil Infrastructure**, **3. Substructure & Foundations**, **4. Superstructure & Shell**, **5. Interior Rough-Ins & MEP Systems**, **6. Interior Finishes & Equipment**, **7. Commissioning, Closeout & Turnover** — plus two cross-cutting tracks: **Professional Skills** (the management/leadership/financial/commercial layer a PM carries through every phase — including the Integrated-PM material from Fewings & Henjewele's *Construction Project Management: An Integrated Approach*, 3rd ed.) and **Workflows & Concepts** (how to run SiteCommand's tools — RFIs, Submittals, Buyout, Change Events, Commitments, Cost & Billing, Field Ops, Budget, Scheduling, Quality, Safety, Risk — together with the foundational construction literacy every phase assumes: reading drawings/specs, RCP, MEP, CSI, lifecycle, contract types, retainage, plus the contracts/commercial admin material and the master construction glossary). It is distinct from Best Practice Templates (company-specific policy that feeds the AI) and from Practice (the hands-on sandbox) — Lessons is the read-and-learn curriculum that prepares a user for both.

Note: this is a reorganization of the earlier nine-track (Workflows / Concepts / Integrated PM / Building the Work / Site & Civil / MEP Systems / Contracts & Commercial / Professional Skills / Preconstruction & Field Ops) taxonomy. **Every lesson id was preserved** — the old technical/site/MEP/management/commercial/fieldops lessons were re-tracked into the lifecycle phases (so quiz keys, schedule `lessonIds`, related-lesson links, and skill mappings all still resolve); only `track`/`category` fields changed, plus 10 new lessons were added to fill phase gaps.

### Content Model
- Lesson content is **static and curated**, not user-editable or database-stored. Types + helpers + the original 16 core lessons live in `lib/training-lessons.ts`; the rest is split across sibling files — `lib/training-lessons-process.ts`, `-management.ts`, `-technical.ts`, `-sitework.ts`, `-mep.ts`, `-commercial.ts`, `-foundations.ts`, `-fieldops.ts`, and `-phases.ts` (the 10 new phase fill-in lessons) — each importing only *types* from the main file (no runtime cycle) and aggregated there into `LESSONS`. **A file no longer maps one-to-one to a track**: each lesson declares its own `track`/`category`, and the phase tracks are assembled by filtering across files. `TRACK_CATEGORY_ORDER` in `lib/training-lessons.ts` pins the subsection order within each track (categories not listed fall to the end in appearance order), and `lessonCategories(track)` sorts by it. Each `Lesson` has an `id`, a `track` (`"precon" | "sitedev" | "substructure" | "superstructure" | "interior-mep" | "finishes" | "closeout" | "foundations" | "workflow"` — the `foundations` and `workflow` keys are reused for Professional Skills and Workflows & Concepts), a `category`, `title`, `summary`, estimated `minutes`, optional `keyTerms`, a `body` of ordered content blocks (`heading` + `paragraphs`/`bullets`/`ordered`), optional `relatedLessonIds`, and optional `links`.
- 102 lessons ship today (92 original + 10 new): **Pre-Construction & Entitlements** 16 (feasibility/zoning, design & engineering, budgeting & procurement, permitting — draws in the old site-analysis/entitlements/ESA, design-side, geotech, takeoffs, constructability/VE, BIM, delivery-method/AIA, long-lead, procurement-strategy, business-case, Div 00/01 lessons + `wf-permits`), **Site Development & Civil Infrastructure** 12 (survey/layout/mobilization, erosion control & demolition, earthwork/grading, underground utilities, hardscape/ADA, landscape/wetlands), **Substructure & Foundations** 4 (deep foundations, shallow foundations, slab-on-grade/concrete), **Superstructure & Shell** 4 (structural frame, elevated slabs, building envelope), **Interior Rough-Ins & MEP Systems** 13 (interior framing, the MEP pattern, wet systems & fire, HVAC, power & signal, rough-in coordination), **Interior Finishes & Equipment** 6 (finishes sequence, insulation/drywall/paint, ceilings/flooring, millwork/doors/hardware, MEP trim-out & elevators), **Commissioning, Closeout & Turnover** 5 (systems commissioning, punch list, final inspections & CO, handover), **Professional Skills** 15 (money/estimating, people & leadership, managing the project, rules of the game, capstone case studies — the folded-in Integrated-PM material; six lessons carry real-project "Case in point" blocks — King Shaka Airport, Considerate Constructor Scheme, London 2012, Hong Kong NEC3, Heathrow T5, high-performing sustainable buildings), and **Workflows & Concepts** 27 (the SiteCommand tool workflows + drawings/specs/CSI/lifecycle/building-systems concepts + contracts & commercial admin + the master glossary). The 10 new lessons are `site-demolition`, `sub-deep`, `sub-sog`, `sup-elevated`, `im-framing`, `fin-drywall-paint`, `fin-ceilings-flooring`, `fin-millwork`, `fin-mep-trim`, `co-inspections-co`. Existing id prefixes are unchanged (`wf-`, `cn-`, `pm-`, `tech-`, `sc-`, `mep-`, `com-`, `pf-`, `fo-`), so a prefix no longer implies a track.
- Helpers: `getLesson(id)`, `lessonsByTrack(track)`, `lessonCategories(track)`, `TRACK_LABELS`, `TRACK_CATEGORY_ORDER`.

### Simulation Tie-In (Recommended Lessons per Day)
- `TrainingDay` in `lib/training-schedule.ts` carries optional `lessonIds` — every scheduled PM day (Days 1-7, 10, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77) lists 3-4 lessons matched to that day's task batch (e.g. Day 14 "Foundations & Site Utilities" → grading plans, sitework, foundations, concrete).
- `TrainingDayPanel` resolves them via `getLesson` (unknown ids silently dropped) and renders a green **📖 Recommended lessons** card directly below the **Message from your coach** section (above the task list), deep-linking each to the standalone module page `/training/lessons/<id>` in a new tab.
...
### End-of-Module Quizzes (Graded + Recorded)
- Every module ends with a short multiple-choice quiz (**3 questions each — except the `pm-cases` capstone, which has 5 scenario questions — 308 total**). Questions + correct answers are static/curated in `lib/training-lesson-quizzes.ts`, keyed by `Lesson.id`, mirroring the lessons themselves. Helpers: `getLessonQuiz`, `getPublicQuiz` (answer-stripped, safe for the client), `hasQuiz`, `gradeQuiz`.
- **Answers never reach the browser**: the quiz module holds the correct-answer indices, so it is imported only from server code (the lesson page's server component via `getPublicQuiz`, and the quiz API route via `gradeQuiz`). `LessonDetailClient` imports it **type-only** (`import type { PublicQuiz }`), which is erased at compile time. Grading is server-side.
- Schema: `training_lesson_quiz_results` (migration `175_training_lesson_quiz_results.sql`) — `user_id`, `lesson_id`, `score`/`total` (latest attempt), `best_score`, `attempts`, `completed_at`, `UNIQUE(user_id, lesson_id)`.
- API: `GET /api/training/lessons/quiz` → `{ results: { [lessonId]: { score, total, bestScore, attempts, completedAt } } }`. `POST /api/training/lessons/quiz` body `{ lessonId, answers: number[] }` → grades server-side, upserts the result (carrying forward `best_score` = max and incrementing `attempts`), returns `{ score, total, correct[], correctAnswers[], bestScore, attempts }`. Unanswered questions are normalized to `-1` (wrong).
- The recorded grade shows back on the **Training Modules page** as a per-module badge (`Quiz {bestScore}/{total}`, green when perfect, "Quiz not taken" otherwise) alongside the completion checkmark.
...
- `app/training/lessons/page.tsx` (session gate + legacy `?lesson=` redirect) + `LessonsClient.tsx` — the module **library**: a left-nav curriculum tree of the nine lifecycle tracks (Pre-Construction & Entitlements / Site Development & Civil Infrastructure / Substructure & Foundations / Superstructure & Shell / Interior Rough-Ins & MEP Systems / Interior Finishes & Equipment / Commissioning, Closeout & Turnover / Professional Skills / Workflows & Concepts) with per-category progress bubbles, a category-grouped list where each module is a new-tab hyperlink with a completion checkmark + quiz-grade badge, and header progress counters (modules complete, quizzes taken, average quiz grade). Because modules are completed in **other tabs**, the list silently refetches progress + grades on window `focus`/`visibilitychange` so badges are never stale.
...
### Branding
- The former amber **SiteCommand Training** top banner was **removed** at the user's request. `TrainingBanner` (`app/projects/[id]/components/TrainingBanner.tsx`) is now a **headless** component (renders `null`) that only drives the sandbox auto-save; it's still mounted from `app/projects/[id]/layout.tsx` when `is_training` is true. The only remaining training chrome is the left-edge **Day panel** (and its collapsed hamburger trigger). To exit a sandbox, use the panel / navigate back to `/training/practice`.
...
- The headless `TrainingBanner` (client component) auto-saves on a **60s heartbeat** and on **tab hide** / **page close** (`navigator.sendBeacon`, which carries session cookies through unload). With the visible banner removed, there is no longer a "Save progress" button or an on-screen "All changes saved" indicator — the checkpoint runs silently.
...
- **Day-by-day task panel** — `app/projects/[id]/components/TrainingDayPanel.tsx`, mounted from `app/projects/[id]/layout.tsx` when `is_training` **and** the role is a `SimRole`. A collapsible **left-edge** panel titled **Day {n}** that surfaces the tasks scheduled for the trainee's current in-sim day (`projects.training_day`), with checkable tasks (each shows a category chip, collaborators, and the deliverable), a phase/timeframe sub-header, and a **Complete Day {n} →** button that advances to the **very next day** (`training_day + 1`). The day's **coach message** (see "Coach Narrator") renders as an embedded collapsible section at the top of the panel body, so all training guidance lives in this single surface. **Day 1** also pins the role-specific **Company Onboarding** PDF link at the top.
  ...
  - **Coach** narrates only on days that carry a task batch — the in-between (empty) days surface no coach section (`TrainingCoachSection` gates on `getScheduledDay`).
  - **State** — task check-offs persist per project in `localStorage` (one JSON map `sc-training-tasks-{projectId}` keyed `"{day}-{index}"`), as does the collapsed flag and the Day-1 onboarding-read flag, all via `useSyncExternalStore` (SSR-safe). Collapses to a neutral **hamburger icon button** on the left edge (styled like the dashboard's menu button — gray icon on white, no orange — with an indigo pulse dot while the day's coach message is unheard); `z-40` so it sits below modals and clears the bottom-right Assist FAB.
...
### Coach Narrator (Text)
A text "coach" walks the trainee through the project — embedded as a compact collapsible **section inside the Day panel** (no separate floating card or popup, to keep the sandbox calm), directly above the **Recommended lessons** card. On each scheduled in-sim day the section sits expanded with a **"New"** chip and loads that day's message automatically — no audio, no play step. Day 1 is the full welcome monologue (orientation, the "no safety net" reality, the project network); every later scheduled day is a short, phase-aware briefing. Scoped to **Project Manager** sandboxes (the only narrated role today). The coach previously spoke via ElevenLabs-synthesized audio; that integration was removed and the coach now defaults to plain text.

- **Scripts** — `lib/training-narration.ts` (client-safe) is the single source of truth. `buildTrainingNarration(role, day, { userName, projectName })` returns `{ title, text }`: the welcome script (with the trainee's first name + a cleaned project name — strips a leading `Training: ` prefix) on day one, then a hand-authored briefing per scheduled PM day (Days 2-7, 10, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77), with a schedule-derived fallback (`getTrainingSchedule` phase/timeframe + top tasks) for any unauthored day. Both the Coach component and the API route build from this one module, so the message always matches.
- **API** — `POST /api/projects/[id]/training/narration` body `{ day }`. Requires a session + `canAccessProject`; the project must be `is_training`. Resolves the raw `day` to the active scheduled day (`resolveDayIndex`), looks up the trainee's first name (`users`) and the project name, and returns `{ title, text, day }`.
- **UI** — `app/projects/[id]/components/TrainingCoach.tsx` exports `TrainingCoachSection`, an embeddable section rendered at the top of the Day panel body by `TrainingDayPanel` (which passes the active day as a prop; the section renders nothing on days without a scheduled task batch). Unheard days start expanded with a **"New"** chip and fetch + display the message text automatically; once acknowledged ("Got it — let's get to work") the day is recorded and the section folds to a single quiet row that can be reopened to re-read anytime. While the whole Day panel is collapsed, its left-edge tab shows a **pulse dot** when the day's message is unheard.
  - **Day sync** — the active day is passed directly as a prop from `TrainingDayPanel` (the old `localStorage["sc-training-active-day-{projectId}"]` bridge was removed with the standalone card), so completing a day surfaces the next coach message in place. Heard days persist in `sc-training-coach-heard-{projectId}` (JSON array, same key as the old card so prior acknowledgements carry over), read via `useSyncExternalStore` — the panel reads the same key for its collapsed-tab dot.
...
### Graded Deliverable Templates (Excel Workbooks → Submit → Email → Grade)
Scheduled tasks whose deliverable is a **document** (gap log, bid leveling sheet, submittal register, SOV, pay app, owner report, closeout tracker, …) are backed by a downloadable **Excel template** and a graded submission loop: download the template from the Day panel/workspace → fill it out in Excel → submit the completed workbook → SiteCommand sends the simulated **follow-up email** (file attached) to the named project personas where the deliverable defines one → the workbook is **AI-graded** against curated criteria → the **grade badge shows next to the task in the Day panel**. 20 deliverables ship across the PM schedule (Days 1, 2×2, 3×3, 5, 6×2, 10, 14, 21×2, 35, 49×2, 56, 63, 70, 77).

- **Definitions** — `lib/training-deliverables.ts` (client-safe): `TrainingDeliverable` keyed by `role + day + taskMatch` (exact task string, same pattern as meetings) with `templateFile`/`templateLabel`, `columns` + `sampleRow` (drive both the generated template and the grader), `instructions`, optional `followUp` (`summary` revealed on check-off, `emailSubject`, `recipients` = INBOX_SENDERS keys, `hcRecipients` healthcare override), 4 grading `criteria` each with degraded-mode `keywords`, and optional `facts(projectType)` grader grounding (e.g. the Day-1 bid tab via `meetingsForType`, the sub roster via `subsForType`). Helpers: `deliverableForTask`, `getTrainingDeliverable`, `resolveDeliverableRecipients` (pack-aware persona resolution), `deliverableConversationId`, `letterForScore`, `gradeBadgeClass`.
- **Templates** — generated `.xlsx` files in `public/training/templates/` (one per deliverable: title + how-to + instruction bullets + header row + a marked example row). Regenerate after editing definitions: `npx tsx scripts/generate-training-templates.ts` (SheetJS, already a dependency).
- **Grading** — `lib/training-deliverable-grading.ts` (server): `extractWorkbookText` (SheetJS → CSV text; strips template boilerplate + the untouched example row; accepts .xlsx/.xls/.xlsm/.csv), `gradeDeliverableSubmission` (Gemini `gemini-2.5-flash` structured output → `{ score 0-100, summary, strengths, gaps, criteria[met/note] }`; keyword fallback without `GEMINI_API_KEY` — an untouched template grades 25/F), plus `buildDeliverableEmailHtml` (trainee's outbound email with the file linked) and `buildRecipientAckHtml` (the lead recipient's **grade-aware** acknowledgment — strong work gets a clean thank-you, weak work gets pushback naming the top gap; no extra model call).
- **Schema** — `training_deliverable_submissions` (migration `180`): one row per `(project, deliverable)` — file name/storage path, `sent_to`, `extracted_text`, `score`/`letter`, `feedback` JSONB, `attempts`; resubmitting upserts (regrades in place, attempts++).
- **API** (owner-only, training-only) — `GET /api/training/projects/[projectId]/deliverables` (list, backs the Day-panel badges); `GET/POST …/deliverables/[deliverableId]` (POST = multipart `file` + `note`, `maxDuration 60`: extract → store in `project-drawings/{projectId}/_deliverables/` → grade → write the email thread `training-deliverable-{id}` + outbound message from the trainee + ack reply into `project_email_threads`/`messages` (best-effort) → upsert submission); `GET …/deliverables/[deliverableId]/file` (signed-URL redirect to the submitted workbook — the stable link embedded in the email).
- **Workspace** — `/training/deliverable?project={id}&d={deliverableId}` (`app/training/deliverable/`, server gate mirrors the meeting page: owner + training + role match). Client: template download, instructions + column chips, upload + optional email note, the follow-up recipients card, submit → grade view (letter badge, per-criterion ✓/✕ with notes, gaps, links to the submitted file + the sent email in project Emails), **Improve & resubmit**. On a successful grade the matching Day-panel task auto-checks via `sc-training-tasks-{projectId}` (cross-tab storage event, same as meetings).
- **Day panel** — `TrainingDayPanel.tsx` calls `deliverableForTask` per task: a green "📄 {template} — open workspace ↗" link under the task, the **grade letter chip** next to the task title once graded (`gradeBadgeClass` colors, tooltip shows score/100), and — when the task is **checked off but not yet submitted** — an amber "Follow-up before this counts" card revealing the deliverable's email instructions with a workspace link. Grades come from the list endpoint, fetched on mount and refreshed on window `focus`/`visibilitychange` (grading happens in the workspace tab).
...
- **Inbox** — `HC_INBOX_SENDERS` (the VA cast: by-the-book **Contracting Officer** Karen Whitlock, overworked **COR** Reginald Foster who can't authorize changes, Infection Control RN Diane Alvarado, VA Police badging, hospital Facilities, the A/E, a critical-power vendor, special inspections) and `HC_INBOX_EMAILS` (17 day-keyed emails carrying federal / occupied-hospital gotchas, incl. a patient-ward noise-complaint plant).
- **Scenarios** — `HC_SCENARIOS` (13 planted decisions with miss/handle ripples): VA **badging** lead time, **ICRA** permit before work, the **85% subcontracting** limitation, planned-**outage MOP**, **Davis-Bacon certified payroll**, an unforeseen-**asbestos differing site condition** (REA, not a verbal COR direction), **medical-gas ASSE certification**, the **critical-power release** (38-week lead / 30-day hold), **noisy-work windows** with the hospital, **smoke-barrier firestop** correction before cover, the **containment-rental invoice dispute**, the **verbal-direction / constructive-change** trap, and the **ILSM fire watch** during the sprinkler impairment.
...
### Overview
The training program grades the trainee's **actual decisions**, not just quizzes. Planted inbox decisions ("scenarios") are evaluated handled/missed, every graded signal rolls up into a **per-skill competency profile**, and an eligible trainee can issue a **verifiable public credential** ("SiteCommand Certified") with a certificate PDF — the train → prove → get-hired loop that feeds the Career Center job board (`/careers`).

### Scenarios (lib/training-scenarios.ts — client-safe content)
- 8 `TrainingScenario`s close the loop on decisions the PM inbox feed plants (`lib/training-inbox.ts`): switchgear long-lead release (price-hold window), owner EV-charging ROM responsiveness, failed trench compaction, sprinkler early-start hold, Bedrock lien-waiver chase, equipment-rental overbilling dispute, mechanical front-loaded billing cut, elevator submittal/fab-slot protection.
- Each carries: `skill` (see taxonomy below), `plantedDay`/`deadlineDay`, canonical `threadSlugs`, a `topics` regex (evidence matching), `expectation` (what good handling looks like — drives the judge and UI), `handledKeywords` (degraded fallback), and optional ripple emails: `consequence` (delivered when missed) and `confirmation` (delivered when handled). Ripple thread conv ids are deterministic: `training-ripple-{scenarioId}-{miss|ok}` (`scenarioConversationId`).

### Evaluation + Consequences (lib/training-scenario-eval.ts — server)
- `runTrainingScenarioEngine(supabase, { projectId, day })` — never throws; PM-role training projects only. Runs from the **day-advance PATCH** (`/api/training/projects/[projectId]`, now `maxDuration = 60`) after inbox delivery, and on demand from `POST /api/training/competency/evaluate` `{ projectId }` (owner-only).
- **Evaluate**: for scenarios with `deadlineDay < day` and no final outcome — evidence = trainee-sent emails (`from_address` = owner email across all sandbox threads; canonical-thread OR topics-regex match) + `tasks` matching topics. No evidence → missed without a model call. Otherwise one Gemini `gemini-2.5-flash` structured-output call judges all due scenarios (`{ outcomes: [{ id, handled, note }] }`); fallback = `handledKeywords` heuristic. Upserts `training_scenario_outcomes` (migration `176`, `UNIQUE(project_id, scenario_id)`: status handled/missed, note, evidence snippet, evaluated_day, consequence_delivered_at).
- **Forgiveness rule**: a "missed" outcome is re-evaluated on later runs while its ripple hasn't been delivered (and only when the evidence text changed) — acting late but before the consequence lands still counts. Once the ripple is out, the outcome is final.
- **Ripple**: when `consequence.day`/`confirmation.day` ≤ current day, the matching email is written as an ordinary stored thread+message (same shape as `deliverTrainingInboxThroughDay`; sender resolved via `INBOX_SENDERS` + `inboxSenderEmail`), then `consequence_delivered_at` stamps. Idempotent via the deterministic conv id (skips insert if the thread exists). `training-seed.ts` now exports `DEFAULT_COMPANY` + `emailDomain` for this.

### Skills Profile (lib/training-skills.ts client-safe + lib/training-competency.ts server)
- Six `SkillKey`s: `buyout`, `cost`, `schedule`, `submittals`, `field`, `comms` (SKILLS defs with labels/descriptions). Mappers: `lessonSkill` (category/title keyword rules, then track default), `checkpointSkill` (explicit map for the bid-review checkpoint ids), `phaseSkill` (schedule phase → skill). Levels via `skillLevel`: <40 Developing, 40-64 Competent, 65-84 Proficient, 85+ Advanced.
- `computeCompetencyProfile(supabase, userId)` aggregates **across all the user's sandboxes (archived included)**: scenario outcomes (weight 0.40), meeting checkpoint catches from `training_meeting_minutes.checkpoints` (0.25), quiz results `training_lesson_quiz_results` best_score/total routed via `lessonSkill` (0.20), phase-review task completion from `training_phase_reviews` completed/missed (0.15). Weights renormalize over components with signal; per-skill evidence bullets (≤8) cite the concrete items. Returns `{ skills, overall, stats, eligibility }`.
- Eligibility rules (all required): quizzes on ≥8 modules, ≥1 meeting completed, ≥3 scenarios graded, ≥1 phase review, overall ≥60.

### Credential (migration `177 training_credentials`)
- `POST /api/training/credential` — issues (or refreshes) the credential when eligible (422 with rules otherwise). One row per user; `code` (`SC-XXXX-XXXX`, unambiguous alphabet, crypto-random) stays stable across re-issues so shared links never break. Snapshot of skills+stats stored in `profile` JSONB. `GET` returns it.
- `GET /api/training/credential/certificate` — landscape certificate PDF (pdf-lib, `lib/training-certificate-pdf.ts`), attachment download.
- **Public verification**: `/verify/[code]` (`app/verify/[code]/page.tsx`, no auth, force-dynamic) renders the stored snapshot — holder, overall level/score, per-skill bars, "how this was earned" stats — or a clear not-found state. On DB failure it errors rather than falsely reporting "not verified".

### APIs + UI
- `GET /api/training/competency` — live profile + credential summary (backs the page below).
- `POST /api/training/competency/evaluate` `{ projectId }` — on-demand engine pass for one sandbox at its current day.
- **Training → Skills & Credential** (`/training/skills`, nav node in `TrainingNav.tsx`): overall score card + stats strip, credential card (eligibility checklist + Issue button before; verify link / copy / certificate download / refresh-snapshot after), expandable per-skill bars with component breakdown + evidence, and a "Grade a sandbox now" list (active PM sandboxes → evaluate POST).
- **Career Center tie-in**: `/careers` silently fetches `GET /api/training/credential`; a logged-in certified visitor sees a green "SiteCommand Certified — include your verification link in applications" banner above the job search.

## Training – Recurring OAC Meetings + Timed Site-Walk Q&A (Days 30/60)

### Overview
Every 30 in-sim days the PM sandbox schedules a **Monthly OAC meeting + site walk** (Days 30 and 60; Day-panel task "Run the monthly OAC meeting and walk the site with the owner and architect", hyperlinked via `meetingForTask`). Attendees are the owner's rep **Elaine Whitfield** and the project architect **Laura Chen** (matching their inbox personas). The sit-down agenda runs through the existing LLM meeting engine (status, cost/changes, look-ahead — each with one hidden checkpoint: Day 30 "EV scope on the potential-change log", Day 60 "billed-vs-installed honesty"); then the meeting continues into a **timed site walk** where the owner/architect quiz the PM about recent/upcoming work ("when does switchgear land?", "what light fixtures go in this lobby?").

### Site-walk mechanic
- `TrainingMeeting.walk` (`lib/training-meetings.ts`): `{ intro, timeLimitSeconds: 30, questions: WalkQuestion[], outro }`. Each `WalkQuestion` carries the asker, the ask (with scene-setting), `fullAnswer` (full-credit criteria), `sourceHint` (half-credit criteria), a `skill`, and fallback keyword lists. 4 questions per OAC.
- **Client-driven + deterministic** (`TrainingMeetingClient.tsx`): when the turn API returns `done` and the meeting has a walk, the client appends the intro + first question and starts a **30-second countdown** (persisted `walkAskedAt` so a reload can't reset the clock; expiry while the tab was closed is caught on mount). Answering advances straight to the next question (no LLM between walk questions); expiry appends a canned "that's time" line from the asker and records the miss. After the last question the outro plays, the task auto-checks, and minutes are generated. The agenda tracker shows a synthetic "Site walk" row with a live Question x/y indicator; the input card turns amber (red ≤10s) with the countdown chip.
- **Grading** (`gradeWalkResponses` in `lib/training-meeting-minutes.ts`, run alongside minutes generation): **full credit (1.0)** — confident, substantively correct on-the-spot answer (judged on plausibility/command, not trivia); **half credit (0.5)** — correctly telling the asker where the info lives (fixture schedule, spec section, testing reports, procurement log) with a follow-up commitment; **none** — expired/vague/wrong. Gemini structured output; degraded fallback: expired→none, fullKeywords→full, sourceKeywords→half, ≥25-word substantive answer→half.
- **Storage** (migration `178`): `training_meeting_minutes.walk_results` JSONB + `walk_points` NUMERIC + `walk_total` INT; minutes payload adds `walkResults/walkPoints/walkTotal`; the saved-minutes view renders a "Site walk Q&A" score card (✓ Full / ◐ Half / ✕ Missed per question, answer + note + full-credit answer on misses); the checkpoint "Meeting effectiveness" card now renders only when `scoreTotal > 0`.
- **Competency**: walk results roll into the meeting-performance component (weight shared with checkpoints) routed by each question's `skill` (full=100/half=50/none=0), with evidence bullets and `walkPoints/walkTotal` stats surfaced on the Skills page.
