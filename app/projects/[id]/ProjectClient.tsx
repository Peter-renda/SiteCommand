...
function ProjectToolsDropdown({ projectId }: { projectId: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
      >
        Project Tools
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[580px] bg-white border border-gray-100 rounded-xl shadow-xl z-50 p-5">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        {loading ? (
          ...
          <>
            {/* Back link + Project Tools dropdown */}
            <div className="flex items-center justify-between mb-8">
              <a href="/dashboard" className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-700 transition-colors">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
                All Projects
              </a>
              <ProjectToolsDropdown projectId={project.id} />
            </div>
