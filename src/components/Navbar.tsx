...
const navItems = [
  { label: "Solutions", items: ["Analytics", "Automation", "Integrations", "Security"] },
          ...
          {navItems.map((item) => (
            <div key={item.label} className="relative">
              <button
                ...
                onClick={() => item.items.length === 0 ? navigate(item.href || "#") : setOpen(open === item.label ? null : item.label)}
              >
                {item.label}
                {item.items.length > 0 && (
              ...
              {item.items.length > 0 && open === item.label && (
                <div className="absolute top-full left-0 mt-1 w-44 bg-white border border-gray-100 rounded-lg shadow-lg py-1" onMouseEnter={() => setOpen(item.label)} onMouseLeave={() => setOpen(null)}>
                  {item.items.map((sub) => (
